"""
Stepwise linear discriminant analysis classifier. Based on stepwisefit from
MATLAB.
XJC 2021
"""
...
class Clfr:
    ...
    def update(self,new_data,new_labels):
        """
        Re-trains classifier with additional training data. Does not update
        'te_labels' according to the new parameters.
        """
    ...
    def test(self,data,labels=[]):
        """
        Apply classifier to testing data. If labels are provided, calculate
        AUC.
        """
    ...
    def copy(self):
        ...
        for key in self.__dict__.keys():
            setattr(clfr,key,self.__dict__[key])
        return clfr

def stepwisefit(data,labels,penter=0.1,premove=0.15,max_iter=60,zmuv_normalize=False):
    """
    Step-wise linear discriminant analysis (SWLDA). Adapted from stepwisefit in MATLAB
    Args:
    - data (num obs x num feats np array of doubles): extracted features
    - labels (num obs x 1 np array of bools or doubles): truth labels
    - zmuv_normalize (bool): whether or not to normalize the data (zero mean unit variance)
    Returns:
    - all_weights (num_feats x 1 np array of doubles): SWLDA model weights
    - model_terms (num_feats x 1 np array of bools): features to include in model
    """
...
def stepcalc(data,labels,model_terms):
    ...
    r2 = mb.repmat(yr[:,None],1,np.sum(np.invert(model_terms)))-np.multiply(
        xr,mb.repmat(b2,N,1))
    df2 = max(0,dfe-1)
...
def safedivide(numer,denom):
    ...
    if not np.any(t) or not numer.size:
        ...
        if np.size(numer)==1 and np.size(denom)>1:
            numer = mb.repmat(numer,np.size(denom))
        elif np.size(denom)==1 and np.size(numer)>1:
            denom = mb.repmat(denom,np.size(numer))
            t = denom==0
